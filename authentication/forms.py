...
class EditProfileForm(forms.Form):
    ...
    def clean(self):
        print(self.cleaned_data['email'] + ' | ' + self.user_mail)
        if self.cleaned_data['email'] != self.user_mail and User.objects.filter(
                email=self.cleaned_data['email']).count() != 0:
            raise ValidationError(_('The given e-mail address is already in use by another user.'))
...
class ChangePasswordForm(forms.Form):
    ...
    """
    old_passwd = forms.CharField(required=False, label=_("Current Password"), widget=forms.PasswordInput,
                                 help_text=_("Leave clear to keep your current password."))
    new_passwd_1 = forms.CharField(required=False, label=_("New Password"), widget=forms.PasswordInput,
    ...
    def clean(self):
        # Check whether user wants to change the password
        # Check whether new passwords do match
        ...
        # run password validators specified in setting.py
        validate_password(self.cleaned_data['new_passwd_1'])
        if authenticate(email=self.user_mail, password=self.cleaned_data['old_passwd']) is None:
...
class RegisterForm(forms.Form):
    ...
    first_name = forms.CharField(required=True, label=_("First Name"))
    last_name = forms.CharField(required=True, label=_("Last Name"))
    passwd_1 = forms.CharField(required=False, label=_("Password"), widget=forms.PasswordInput,
                               help_text=password_validators_help_text_html())
    passwd_2 = forms.CharField(required=False, label=_("Confirm Password"), widget=forms.PasswordInput)

    def clean(self):
        # Check whether passwords do match
